...
do
  ...
  -- Hide the command-line when not in use, forcing it to blend
  -- into the statusline for a clean, single-bar look.
  vim.opt.cmdheight = 0

  -- Manual float trigger that jumps your cursor INSIDE the window
  vim.keymap.set('n', '<leader>d', function() vim.diagnostic.open_float { focus = true } end, { desc = 'Show line [D]iagnostics and focus' })
end
...
do
  ...
  -- The "Magic" line: This makes other plugins think mini.icons
  -- is actually nvim-web-devicons, so they work without errors.
...
do
  ...
  --
  -- We are using snacks.picker as our blazing fast fuzzy finder!
  ...
  -- Snacks.picker integration to search through your entire notification history
  vim.keymap.set('n', '<leader>sM', function() Snacks.picker.notifications() end, { desc = '[S]earch [M]essages (History)' })
